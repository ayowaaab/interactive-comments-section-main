...
const IconButton = ({ text, icon }: { text: string; icon: string }) => {
  return (
    <div className="hover:opacity-50 cursor-pointer flex items-center gap-2">
      ...
      {text === "Delete" ? (
        <p className="text-xs md:text-sm lg:text-lg text-primary-softRed font-medium">{text}</p>
      ) : (
        <p className="text-xs md:text-sm lg:text-lg text-primary-moderateBlue font-medium">{text}</p>
      )}
    </div>
  );
};
const SimpleButton = ({ text }: { text: string }) => {
  return (
    <button className="text-xs md:text-sm lg:text-lg bg-primary-moderateBlue text-white font-medium rounded-lg py-3 px-6 hover:opacity-50 uppercase">
      {text}
    </button>
  );
};
